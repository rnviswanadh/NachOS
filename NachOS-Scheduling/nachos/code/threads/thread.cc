...
NachOSThread::Exit (bool terminateSim, int exitcode)
{
    ...
    if(status == RUNNING){
      if(stats->totalTicks - burstStart > 0){
        // printf("zero burst\n");
        numberOfBursts++;
      }
      burstEstimate = (burstEstimate + stats->totalTicks - burstStart)/2;
      totalBurst = totalBurst + stats->totalTicks - burstStart;
      cpuDelay = cpuDelay + stats->totalTicks - burstStart;
      threadCPUburst[pid] = totalBurst;
    }
    status = BLOCKED;
    // printf("exiting before update readyList count in list %d\n", scheduler->readyList->count);
    scheduler->UpdatePriority();
    // printf("exiting after update readyList count in list %d\n", scheduler->readyList->count);
    // update priority of each thread by using their priority and nice values in readyList and sleepqueuehead
    // if(algo==4){
    //   TimeSortedWaitQueue *ptr = sleepQueueHead;
    //   while(ptr!=NULL){
    //     NachOSThread* t = ptr->GetThread();
    //     t->priority = t->basePriority + t->nice + t->totalBurst/2;
    //     ptr = ptr->GetNext();
    //   }
    //   List * p = scheduler->readyList;
    //   ListElement * start = p->first;
    //   while(start!=NULL){
    //     NachOSThread *t = (NachOSThread *)start->item;
        
    //     t->priority = t->basePriority + t->nice + t->totalBurst/2;
    //     start->key = t->priority;
    //     start = start->next;
    //   }
    // }

    total = total + totalBurst;
    totalW = totalW + totalWait;
    totalBursts = totalBursts + numberOfBursts;
    burstAverage = totalBurst/numberOfBursts;
    running = false;
    printf("\n%s CPU BURST %lld\n", name, threadCPUburst[pid]);
    printf("total %ld totalBursts %ld averageBurst%ld\n", total, totalBursts, total/totalBursts);
    printf("name %s numberOfBursts %d burstAverage %d\n", name, numberOfBursts, burstAverage);
    if(currentThread->GetPID()!=0){
      printf("thisWait %d totalWait %d average %d\n", totalWait, totalW, totalW/(thread_index-1));  
    }
    else
      printf("Main Thread, so no waiting\n");
...
NachOSThread::PutThreadToSleep ()
{
    ...
    scheduler->UpdatePriority();
    // printf("sleep after update readyList count in list %d %d\n", scheduler->readyList->count, currentThread->GetPID());
    while ((nextThread = scheduler->FindNextToRun()) == NULL)
...
void
NachOSThread::setRegistersToZero(){
  for(int i = 0; i<NumTotalRegs; i++){
    userRegisters[i] = 0;
  }
}
