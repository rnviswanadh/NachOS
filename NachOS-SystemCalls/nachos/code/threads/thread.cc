...
//--------------------------------------------------------------------
// insertChild inserts the pid of child to child list of current thread

//--------------------------------------------------------------------
void 
NachOSThread::insertChild(int child_pid){
    struct childNode *p = childList;
    while(p!=NULL){
        p = p->next;
    }
    
    p = new childNode();
    p->childpid = child_pid;
    p->exitCode = -1;
    childList = p;
}

//--------------------------------------------------------------------
// updateparent updates the exit code of the child in childlist 
// of parent of current thread, so that it can be used to check 
// if the child thread is exited

//--------------------------------------------------------------------

void 
NachOSThread::updateparent(int exit_code){
    struct childNode *p = childList;
    while(p!=NULL){
        if(p->childpid == currentThread->getpid()){
            p->exitCode = exit_code;
        }
        p = p->next;
    }  
    if(1){                                      //joinpid==currentThread->getpid()
...
//---------------------------------------------------------------------
// searches if there exist a child of current thread with given join_pid
// if there exists such child, check whether it exists or not 
//
//---------------------------------------------------------------------
int
NachOSThread::searchChild(int joined_pid){
    struct childNode *p = childList ;
    // printf("%d\n", p->exitCode);
    while(p!=NULL){
        // printf("11111111111\n");
        // printf("entering not null\n");
        // printf("%d 11111111 %d\n", p->childpid, currentThread->getpid());
        if(p->childpid == joined_pid){
            return p->exitCode;
        }
        p = p->next;
    }
